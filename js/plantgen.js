        ...
        function renderFlat(flatTree, treePos) {
            ...
            for (var i = 0; i < flatTree.length; i++) {
                ...
                if (config.gravity != 0 || config.windSpeed || config.windTurbulence) {
                    ...
                    if (!branch.weight) {
                        branch.weight =
                            pi * pow(branch.width / 2, 2) * branch.len * woodDensity;
					...
					*/
                    ...
                    bendDistance =
                        (forceScalar *
                            forceAttack *
                            pow(forceAttack * (branch.len / 100), 3)) /
                        (3 * config.elasticity * branch.i_y);
        ...
        function renderAll() {
            ...
            if (flat) {
                for (var i in flatTrees) {
                    renderFlat(flatTrees[i], [
                        (canvas.width / (config.treeCount + 1)) * (~~i + 1),
                        canvas.height,
                    ]);
                }
            } else {
